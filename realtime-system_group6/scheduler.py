...
class BaseScheduler:
    ...
    def evaluate(self, completed_jobs, total_released_count, sim_time, all_released_jobs):
        ...
        '''
        hard_miss_jobs = [j for j in hard_jobs if j.is_missed]
        for j in hard_miss_jobs:
            print("[DBG]", j.name, "rel=", j.release_time, "absD=", j.absolute_deadline,
                "start=", j.start_time, "finish=", j.finish_time,
                "completed=", j.is_completed, "missed=", j.is_missed)
